...
判断交给它起的 agent 或它调的脚本；结果、进度和审批请求以一张原地更新的卡片发回飞书。提问另发一张可填写的卡片。
...
- **提问（`question`）单独发卡片**：Claude `AskUserQuestion` 和 Codex `request_user_input` / `request_user_input_async` 可选择单选、多选或填写自定义答案，一次提交多个问题。独立卡片不会随 turn 结束消失，插件重载后也能依靠 Paseo 的实时 pending 状态作答。其他 provider 和无法识别的题目提示到 Paseo 回答。
...
| 回消息 | 结果、进度、审批请求 | 每条输入一张进度卡原地 patch；提问另发独立卡片 |
...
权限请求（`agent.permission_requested`）当场把进度卡变成「等待审批」；提问另发独立卡片，见上文「审批」；
